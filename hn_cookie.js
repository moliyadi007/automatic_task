...
const cookieName = '红牛能量俱乐部小程序'

const moliyadi = init()

if ($request && $request.method == 'POST' && $request.url.indexOf('redbullapi.rrfenwo.com/wechat/getUserInfo') >= 0) {
    ...
    (err,res,data)=>{
      eval(data)
      update(token,'redbullhd',cookieName,'@')
    }
    );
  }
    }else{
