...
const cookieName = '高通骁友会小程序'

const moliyadi = init()

if ($request && $request.method == 'GET' && $request.url.indexOf('qualcomm.growthideadata.com/qualcomm-app/api/user/info') >= 0) {
    let sessionKey = $request.headers.sessionKey
    let userId = $request.headers.userId
    var token = sessionKey + '#' + userId
    ...
    (err,res,data)=>{
      eval(data)
      update(token,'S_gtxlhCk',cookieName,'\n')
