...
const cookieName = '工友会'

const moliyadi = init()

if ($request && $request.method == 'GET' && $request.url.indexOf('gateway.gyhapp.com/entry/user/getUserInfo') >= 0) {
    let token = $request.headers['lanlinker-satoken']
    ...
    (err,res,data)=>{
      eval(data)
      update(token,'gongyouhd',cookieName,'\n')
    }
    );
  }
    }else{
